...
static void Task_OptionMenuProcessInput(u8 taskId)
{
    ...
    else
    {
        ...
        {
        case MENUITEM_SINGLECATCH:
            previousOption = gTasks[taskId].data[TD_SINGLECATCH];
            gTasks[taskId].data[TD_SINGLECATCH] = SingleCatch_ProcessInput(gTasks[taskId].data[TD_SINGLECATCH]);

            if (previousOption != gTasks[taskId].data[TD_SINGLECATCH])
                SingleCatch_DrawChoices(gTasks[taskId].data[TD_SINGLECATCH]);
            break;
        case MENUITEM_DELETEMON:
            previousOption = gTasks[taskId].data[TD_DELETEMON];
            gTasks[taskId].data[TD_DELETEMON] = DeleteMon_ProcessInput(gTasks[taskId].data[TD_DELETEMON]);

            if (previousOption != gTasks[taskId].data[TD_DELETEMON])
                DeleteMon_DrawChoices(gTasks[taskId].data[TD_DELETEMON]);
            break;
        case MENUITEM_DELETEWO:
            previousOption = gTasks[taskId].data[TD_DELETEWO];
            gTasks[taskId].data[TD_DELETEWO] = DeleteWO_ProcessInput(gTasks[taskId].data[TD_DELETEWO]);
...
static void DeleteMon_DrawChoices(u8 selection)
{
    ...
    DrawOptionMenuChoice(gText_NuzDeleteMonOnFaintOn, 104, YPOS_DELETEMON, styles[0]);
    DrawOptionMenuChoice(gText_NuzDeleteMonOnFaintOff, GetStringRightAlignXOffset(FONT_NORMAL, gText_NuzDeleteMonOnFaintOff, 198), YPOS_DELETEMON, styles[1]);
}
